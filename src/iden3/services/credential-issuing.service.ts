...
export class CredentialIssuingService implements OnModuleInit {
  ...
  async issue(opts: {
    ...
    await this.persistence.saveCredential({
      holder: opts.credentialSubject.id,
      document: credentialDoc,
      nonce: credentialDoc.credentialStatus.revocationNonce!.toString(),
      createdAt: new Date(),
    });
  ...
  async credentialStatus(nonce: string) {
    ...
    return {
      mtp: {
        ...
        existence: await this.persistence.isRevoked(nonce),
      },
      issuer: {
        state: state.hex(),
  ...
  async revoke(nonce: string) {
    this.assertSetupState();
    return this.persistence.revoke(nonce);
  }

  async isRevoked(nonce: string): Promise<boolean> {
    return this.persistence.isRevoked(nonce);
  }
